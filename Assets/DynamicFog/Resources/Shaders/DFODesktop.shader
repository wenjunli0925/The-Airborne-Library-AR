Shader "DynamicFog/Image Effect/Orthographic Desktop" {
	...
	inline fixed4 computeSkyColor(fixed4 color, float3 worldPos) {
	...
	inline fixed4 computeGroundColor(fixed4 color, float3 worldPos, float depth) {
		...
		fixed4 fogColor = lerp(_FogColor, _FogColor2, saturate(worldPos.y / fogHeight));
	 	fogColor = lerp(color, fogColor, groundColor);
		fogColor.a = groundColor;
		return fogColor;
	}
	...
	fixed4 frag (v2f i) : SV_Target {
		...
		#endif
		if (depth01>0.999) {
	    	color = computeSkyColor(color, getWorldPos(i, depth01));
		}  else if (depth01<_FogDistance.z) {
	    	float3 worldPos = getWorldPos(i, depth01);
			worldPos.y -= _FogHeightData.y + 0.00001;
	    	if (worldPos.y>_FogHeightData.z && worldPos.y<_FogHeightData.x+_FogNoiseData.y) color = computeGroundColor(color, worldPos, depth01);
		}
		#if DITHER_ON
