Shader "DynamicFog/Image Effect/Fog And Sky" {
	...
	inline fixed4 computeGroundColor(fixed4 color, float3 worldPos, float depth) {
		...
		fixed4 fogColor = lerp(_FogColor, _FogColor2, saturate(worldPos.y / fogHeight));
	 	fixed4 finalFogColor = lerp(color, fogColor, groundColor);
		finalFogColor.a = groundColor;
	...
	fixed4 frag (v2f i) : SV_Target {
		...
		if (depth>0.999) {
	    	...
	    	if (worldPos.y>_FogHeightData.z && worldPos.y<_FogHeightData.x+_FogNoiseData.y) {
                color = computeGroundColor(color, worldPos, depth);
            }
		}
		#if DITHER_ON
