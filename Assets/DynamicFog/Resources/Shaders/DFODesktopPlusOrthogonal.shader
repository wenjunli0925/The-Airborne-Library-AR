Shader "DynamicFog/Image Effect/Orthographic Desktop Plus Orthogonal" {
	...
	half4 getFogColor(float2 uv, float3 worldPos, float depth, fixed4 color) {
		...
		fogColor *= voidAlpha;
	 	half4 finalFogColor = color * (1.0 - fogColor.a) + fogColor;
		finalFogColor = fogColor.a;
	...
	fixed4 frag (v2f i) : SV_Target {
		...
		color.a = 0;
		float depth = UNITY_SAMPLE_DEPTH(UNITY_SAMPLE_SCREENSPACE_TEXTURE(_CameraDepthTexture, i.depthUV));
		#if UNITY_REVERSED_Z
			depth = 1.0 - depth;
		#endif
