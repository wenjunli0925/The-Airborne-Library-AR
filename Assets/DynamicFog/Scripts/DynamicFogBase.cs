...
namespace DynamicFogAndMist {
    ...
    public class DynamicFogBase : MonoBehaviour {
        ...
        public bool skyboxBlending {
            get { return _skyboxBlending; }
            set {
                if (value != _skyboxBlending) {
                    _skyboxBlending = value;
                }
            }
        }

        const string DYNFOG_SKYBOX_RENDERER = "DynamicFogSkyboxRenderer";
        const string SKW_SKYBOX_BLENDING = "SKYBOX_BLENDING";
        ...
        public static RenderTextureDescriptor rtDesc;
...
#if UNITY_EDITOR
                if (_skyboxBlending && fogMat != null) {
                    if (currentCamera.cameraType == CameraType.SceneView) {
                        fogMat.DisableKeyword(SKW_SKYBOX_BLENDING);
                    } else {
                        fogMat.EnableKeyword(SKW_SKYBOX_BLENDING);
                    }
                }
#endif
            }
        ...
        protected void UpdateMaterialPropertiesNow() {
            ...
            switch (effectType) {
                ...
                case FOG_TYPE.DesktopFogPlusWithSkyHaze:
                    if (fogMatVol == null) {
                        ...
                        fogMatVol = Resources.Load<Material>(matName);
                        if (fogMatVol == null) return;
                        fogMatVol = Instantiate(fogMatVol);
            ...
            if (_skyboxBlending) {
                shaderKeywords.Add(SKW_SKYBOX_BLENDING);
            }
        ...
        void CheckSkyboxRenderer() {
            if (currentCamera == null || currentCamera.cameraType == CameraType.SceneView) return;
            Transform t = transform.Find(DYNFOG_SKYBOX_RENDERER);
            if (t == null && _skyboxBlending) {
                GameObject go = Instantiate(Resources.Load<GameObject>("Prefabs/DynamicFogSkyboxRenderer"));
                go.name = DYNFOG_SKYBOX_RENDERER;
                go.transform.SetParent(transform, false);
            } else if (t != null && !_skyboxBlending) {
        ...
        void SetSkyData() {
            ...
            Vector4 skyData = new Vector4(_skyboxBlending ? 0 : _skyHaze, _skySpeed, _skyNoiseStrength, currentSkyHazeAlpha);
        ...
        /// <summary>
        /// Returns the alpha value of fog of war at a given world space position
        /// </summary>
